...
const EmojiCounter = ({ emoji, count, onClick }) => {
  ...
  return (
    <button
      ...
      <span
        className={`mx-1 text-xs text-black sm:ml-2 sm:text-sm md:text-base ${isAnimating ? "emoji-pulse" : ""}`}>
        {count}
      </span>
      <span
      ...
      <span className="mx-1 text-xs text-black sm:ml-2 sm:text-sm md:text-base">
        {label}
      </span>
    </button>
  );
